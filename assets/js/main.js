;(function(win, doc, $) {
	...
	Loading.prototype.opening = function() {
		if(!win.location.hash) {
			if(this.isSupportSvg) {
				$.Velocity.hook($('.nav-about .back'), 'rotateX', '90deg');
				$.Velocity.hook($('.nav-event .back'), 'rotateX', '90deg');
				$.Velocity.hook($('.nav-contact .back'), 'rotateX', '90deg');
				$.Velocity.hook($('.nav-gallery .back'), 'rotateX', '90deg');
				...
				$.Velocity.hook($('.nav-about, .nav-top'), 'top', '-200%');
				$.Velocity.hook($('.nav-event, .nav-right'), 'left', '200%');
				$.Velocity.hook($('.nav-contact, .nav-bottom'), 'top', '200%');
				$.Velocity.hook($('.nav-gallery, .nav-left'), 'left', '-200%');
			}

			this.$overlay.velocity({opacity: 0}, {duration: 800, delay: 3000, complete: function(el) {
				$(el).hide();
				$('.tilt').addClass('loaded');
				$('.nav-about .back, .nav-contact .back, .nav-event .back, .nav-gallery .back').velocity({rotateX: 0}, {duration: 400, delay: 500, easing:'ease-out' });
				setTimeout(function(){
					setWidthAndHeight();
				},2000);
			}});
...
;(function(win, doc, $) {
	...
	var PageSlider = function() {
		...
		this.$nav_sns 		= $('#nav-contact');
		this.$nav_media		= $('#nav-gallery');
		this.$nav_profile	= $('#nav-about');
		this.$nav_fanclub	= $('#nav-event');
	...
	PageSlider.prototype.onClick = function(e) {
		...
		switch(settings['in']) {
			...
			case '.nav-about':
				...
				$('.nav-event').velocity({translateX: 0}, {duration: NAVI_SPEED, delay: NAVI_SPEED});

				$('.nav-bottom').velocity({translateY: 200+'%'}, {duration: NAVI_SPEED});
				$('.nav-contact').velocity({translateY: 0}, {duration: NAVI_SPEED, delay: NAVI_SPEED});

				$('.nav-left').velocity({translateX: -200+'%'}, {duration: NAVI_SPEED});
				$('.nav-gallery').velocity({translateX: 0}, {duration: NAVI_SPEED, delay: NAVI_SPEED});
			...
			case '.nav-contact':
				...
				$('.nav-about').velocity({translateY: 0}, {duration: NAVI_SPEED, delay: NAVI_SPEED});
		...
		if(hash) {
			if($(hash).length) {
				switch(hash) {
					case '#about':
						setTimeout(function() {
							$('.nav-about a').trigger('click');
						}, 500);
						break;
					case '#event':
						setTimeout(function() {
							$('.nav-event a').trigger('click');
						}, 500);
						break;
					case '#media':
						setTimeout(function() {
							$('.nav-gallery a').trigger('click');
						}, 500);
						break;
					case '#sns':
						setTimeout(function() {
							$('.nav-contact a').trigger('click');
						}, 500);
						break;
				}
			}
